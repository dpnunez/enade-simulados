...
- [ ] Coletar nick no formulario de aceite do convite.
- [ ] Persistir o nick em `User.name`.
- [ ] Garantir unicidade de `User.name`.
- [ ] Garantir que `User.name` nunca seja derivado do email no aceite do convite.
- [ ] Validar `name` no cliente e no servidor com `zod`.
- [ ] Exibir `name` como identidade primaria do usuario onde fizer sentido.
- [ ] Cobrir regras criticas com testes unitarios/integracao e o fluxo principal com E2E.
...
**Independent Test**: Open a generated invite link for `student@example.com`, fill nick `maria_silva` and password, complete registration, then verify login succeeds and the created user has `name = "maria_silva"`, not `student`.
...
3. WHEN a submitted nick only differs by case or surrounding spaces from an existing name THEN system SHALL treat it as the same nick.
4. WHEN an acceptance request bypasses the browser form THEN server-side validation SHALL still enforce name/nick rules.
5. WHEN existing seed users exist THEN they SHALL keep deterministic unique `name` values or be updated deterministically if needed.
...
| --- | --- | --- | --- |
| NAME-01 | P1: Convidado Define Nick no Aceite | Service + UI | Planned |
| NAME-02 | P1: Sistema Valida e Unifica Nick no Campo Name | Data + Schema + Service + Tests | Planned |
| NAME-03 | P2: Interface Usa Name como Identidade Primaria | UI | Planned |
| NAME-04 | Edge Cases: trimming/validation/uniqueness/no-email-derivation | Data + Tests | Planned |

**Coverage:** 4 total, 4 mapped to planned work, 0 verified.

## Success Criteria

- [ ] Convidado precisa informar nick para aceitar convite.
- [ ] Usuario criado por convite tem `name` persistido a partir do formulario.
- [ ] `User.name` e unico.
- [ ] `name` nunca e derivado do email no aceite do convite.
- [ ] Name/nick invalido ou duplicado bloqueia cadastro sem consumir o convite.
