...
- `src/features/invitations/invitation.schema.ts:acceptInvitationSchema` uses the shared password policy from `src/features/auth/password-policy.ts`; invitation passwords must match reset-password requirements and include matching `passwordConfirmation`.
