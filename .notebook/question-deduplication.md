...
- Question edits must not blindly delete and recreate alternatives: `SimulationAnswer` keeps FKs to `QuestionAlternative`, so `src/features/questions/question.service.ts:updateQuestion()` syncs alternatives by position and preserves existing IDs for normal edits; remaining FK conflicts map to `QUESTION_RELATION_IN_USE`
